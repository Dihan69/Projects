#include "include/Enemy_Type_1.h"
...
void Enemy_Type_1::move_enemy(const Environment1& e)
{

    if(enemy_state==left_moving)
    {
        if(e.get_environment1(enemy_pos_y, enemy_pos_x-1)==':')
            enemy_state=right_moving;
        else
            enemy_pos_x--;
    }
    else if(enemy_state==right_moving)
    {
...
void Enemy_Type_1::change_enemy_state(enemy_state_type_1 next_state)
{
    ...
    else if(next_state==right_moving)
    {
        char temp[5][4]=
        {
            "444",
...
enemy_state_type_1 Enemy_Type_1:: get_enemy_state() const
{
    return enemy_state;
}


void Enemy_Type_1:: convert_enemy (int i, int j)
{
...
void Enemy_Type_1::shoot_enemy_bullet(const Environment1& e)
{
    if(enemy_state==left_moving && current_delay==0 )
    {
    ...
    else if (enemy_state==right_moving && current_delay==0)
